...
The renderer is `../src/glyphs.js` — a single ES module, no dependencies, pure 2D canvas. See `../examples/glyphs-demo.html` for a worked example.
...
For a labeled three-up demo (`IDLE` / `THINKING` / `ERROR` side by side), copy `../examples/glyphs-demo.html` from this folder.
