...
|-------|-------------|-----|---------------|
| `.badge-process` | `--text-primary` + border `--text-secondary` | Process / default emphasis | `SESSION_PROCESS` |
| `.badge-resource` | `--interactive` + border `rgba(91,155,246,0.3)` | Resource / reference | `SESSION_RESOURCE` |
| `.badge-monitor` | `--warning` + border `rgba(212,168,67,0.3)` | Monitor / observation | `MAKE_MONITOR` |
| `.badge-steps` | `--success` + border `rgba(74,158,92,0.3)` | Step count, progress | `2 STEPS` |
| `.badge-system` | `--text-disabled` + border `--border-visible` | System / infrastructure | `SYSTEM`, `DKR` |
| `.badge-recurring` | `--text-disabled` + border `--border` | Recurring / meta | `RECURRING` |
| `.badge-type` | `--text-secondary` + border `--border-visible` | Type tag in lists | `STATUS_CHANGE` |
