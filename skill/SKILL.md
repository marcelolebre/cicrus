...
- **`/Component Index.html`** and **`/Component Index v2.html`** (repo root) — Typographic nested index of the component library. v1 is the quiet editorial pass (Space Grotesk names, Space Mono caps section labels, definition-list rhythm). v2 pushes the visual treatment harder: boxed groups with tinted header strips, a Doto display masthead, numbered IDs per entry, name-over-class-hook stacks, and a status-dot cameo. Use either as a printable / scannable reference of what Cicrus ships.
- **`/elements/lms/`** + **`/elements/lms.css`** (repo root) — 26 standalone LMS element fragments (callouts, code, compare, definitions, figure, footnotes, hero, lab, margin-note, modnav, progress, prose, quiz, quote, reflect, reveal, sparkline, steps, summary, table, toc, topbar), each its own HTML so you can open any one in isolation. They share `elements/lms.css`. `/LMS Components.html` is the catalog page that iframes them all into a Figma-style DesignCanvas browser.
- **`/elements/taskboard/`** + **`/elements/taskboard.css`** (repo root) — 14 standalone Task Board element fragments (cards in each column state, column empty/running states, header, footer, sidebar, side-panel, stats, status-dots, toolbar, topbar). Same pattern: shared CSS, one HTML per element, with `/Task Board Components.html` as the catalog page.
